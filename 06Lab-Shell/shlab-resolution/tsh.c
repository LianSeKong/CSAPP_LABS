...
void eval(char *cmdline) 
{
    ...
    sigfillset(&mask_all);  // add all signal 
    sigemptyset(&mask_one); // empty set
    sigaddset(&mask_one, SIGCHLD); // 添加子线程回收信号
    /* parse command line */
...
*  FIND THE NEW JOB OF ST STATUS,  ST -> FG, ST -> BG
   [50 lines]
 */
void do_bgfg(char **argv) 
{   
...
void sigchld_handler(int sig) 
{
    ...
    // WNOHANG: 如果等待集中的子进程均未终止，则立即返回（返回值为 0）。默认行为是暂停调用进程，直到子进程终止；
    while ((pid = waitpid(-1, &status, WNOHANG | WUNTRACED)) > 0) {
        ...
        if (WIFEXITED(status)) {
            ...
            if (verbose)
	            printf("sigchld_handler: job %d stoped\n", pid);
